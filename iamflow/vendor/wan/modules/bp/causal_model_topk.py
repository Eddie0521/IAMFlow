...
from iamflow.vendor.wan.modules.attention import attention
from iamflow.vendor.wan.modules.model import (
    WanRMSNorm,
    rope_apply,
    WanLayerNorm,
...
from iamflow.utils.memory import gpu, get_cuda_free_memory_gb, DynamicSwapInstaller, log_gpu_memory
...
class CausalWanSelfAttention(nn.Module):
    ...
    def dynamic_topk_routing_attention(
        ...
        expanded_indices = top_k_indices.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, chunk_size, D)
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                ...
                local_end_index_bank = kv_bank["local_end_index"].item() + current_end - kv_cache["global_end_index"].item()
                local_start_index_bank = local_end_index_bank - num_new_tokens
            ...
            if sink_tokens > 0:
                ...
                if local_budget > 0:
                    ...
                    local_end_index_bank = min(local_start_index_bank, kv_bank_size)
                    k_bank = bank_k[:, :local_end_index_bank]
                    v_bank = bank_v[:, :local_end_index_bank]
        ...
        del k_sink, v_sink, k_local, v_local, k_bank, v_bank, k_cat, v_cat
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_result = self.self_attn(
            ...
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, cache_start, kv_bank)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _apply_cache_updates(self, kv_cache, cache_update_infos, kv_bank, crossattn_cache, update_bank=True):
        """
                ...
                if update_info["action"] == "roll_and_insert":
                    ...
                    if write_end_index > write_start_index and new_k.shape[1] == (write_end_index - write_start_index):
                        ...
                        if update_bank:
                            with torch.no_grad():
                                if write_end_index_bank <= bank["k"].shape[1]:
                                    # print(write_end_index_bank, bank["k"].shape[1])
                                    # print('insert')
                                    bank["k"][:, write_start_index_bank:write_end_index_bank] = new_k
                                    bank["v"][:, write_start_index_bank:write_end_index_bank] = new_v
                                    # if block_index == 1:
                                    #     print('insert bank', write_start_index_bank, write_end_index_bank)
                                else:
                                    # print('merge')
                                    new_compressed_kv_cache = self.compress_kv_bank(
                                        kv_cache=bank,
    ...
    def compress_kv_bank(
        ...
        memory_budget_in_blocks: int = 6,
        num_prototypes_in_blocks: int = 1,
    ) -> Dict[str, torch.Tensor]:
        """
        Compresses the video KV cache at the BLOCK (latent) level.

        Args:
            kv_cache (Dict): Historical KV cache for one layer.
                'k' and 'v' tensors shape: [B, L_hist_tokens, H, D].
            new_k, new_v (torch.Tensor): New K/V for the generated video chunks.
                Shape: [B, L_new_tokens, H, D].
            crossattn_cache (Dict): Text prompt KV cache.
            tokens_per_block (int): Number of tokens per latent block (e.g., 1560).
            memory_budget_in_blocks (int): Total number of blocks to keep (e.g., 9).
            num_prototypes_in_blocks (int): Number of prototype blocks to generate (e.g., 1).

        Returns:
            Dict[str, torch.Tensor]: The new, compressed KV cache dictionary for this layer.
        """

        # --- Step 0: Prepare Tensors and Block Information ---
        
        hist_k = kv_cache['k']
        hist_v = kv_cache['v']
        text_q = crossattn_cache["k"] # Using text_k as query based on previous discussion

        B, L_hist_tokens, H, D = hist_k.shape
        _, L_new_tokens, _, _ = new_k.shape
        _, L_text, _, _ = text_q.shape

        assert L_hist_tokens % tokens_per_block == 0, "Historical cache length is not a multiple of tokens_per_block"
        assert L_new_tokens % tokens_per_block == 0, "New K/V length is not a multiple of tokens_per_block"
        
        num_hist_blocks = L_hist_tokens // tokens_per_block
        num_new_blocks = L_new_tokens // tokens_per_block
        ...
        # new_k torch.Size([1, 4680, 12, 128])
        new_k_blocks = new_k.view(B, num_new_blocks, tokens_per_block, H, D)
        # torch.Size([1, 3, 1560, 12, 128])
        new_v_blocks = new_v.view(B, num_new_blocks, tokens_per_block, H, D)
        
        all_visual_k_blocks = torch.cat([hist_k_blocks, new_k_blocks], dim=1)
        all_visual_v_blocks = torch.cat([hist_v_blocks, new_v_blocks], dim=1)
        num_total_blocks = num_hist_blocks + num_new_blocks

        # --- Step 1: Compute Saliency Scores at the BLOCK level ---
        
        # Reshape Q and K for attention score calculation
        q_reshaped = text_q.permute(0, 2, 1, 3).reshape(B * H, L_text, D)
        # torch.Size([12, 512, 128])
        # K needs to be flattened from blocks view
        k_reshaped = all_visual_k_blocks.reshape(B, num_total_blocks * tokens_per_block, H, D)
        # print('k_reshaped shape:', k_reshaped.shape)
        k_reshaped = k_reshaped.permute(0, 2, 1, 3).reshape(B * H, num_total_blocks * tokens_per_block, D)
        # torch.Size([12, 18720, 128])

        # Compute attention scores: Q @ K.T
        attn_scores = torch.bmm(q_reshaped, k_reshaped.transpose(1, 2)) / (D ** 0.5)
        # torch.Size([12, 512, 18720])

        # Aggregate scores over text tokens dimension
        importance_scores_per_token = attn_scores.mean(dim=1)
        # torch.Size([12, 1, 18720])

        # Reshape and average over heads
        importance_scores_per_token = importance_scores_per_token.view(B, H, num_total_blocks * tokens_per_block).mean(dim=1)
        
        # --- THIS IS THE KEY MODIFICATION: Aggregate scores per block ---
        # Reshape scores to [B, Num_Blocks, Tokens_per_Block] and average
        importance_scores_per_block = importance_scores_per_token.view(B, num_total_blocks, tokens_per_block).mean(dim=2)
        # Shape of importance_scores_per_block is now [B, num_total_blocks]
        # torch.Size([1, 12])

        
        # --- Step 2: Select Top-K Salient BLOCKS from History ---
        
        num_topk_blocks_to_keep = memory_budget_in_blocks - num_prototypes_in_blocks
        
        if num_topk_blocks_to_keep > 0 and num_hist_blocks > 0:
            # Separate scores for historical blocks
            # hist_block_scores = importance_scores_per_block[:, :num_hist_blocks]
            
            # Find top-k most salient blocks
            _, topk_block_indices = torch.topk(importance_scores_per_block, k=min(num_topk_blocks_to_keep, num_total_blocks), dim=1)
            # torch.Size([1, 8])
            # print('topk_block_indices', topk_block_indices)
            # Gather the selected historical blocks
            # Indices need to be expanded to gather along the block dimension
            topk_block_indices_expanded = topk_block_indices.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, tokens_per_block, H, D)
            
            salient_k_blocks = torch.gather(all_visual_k_blocks, 1, topk_block_indices_expanded)
            # torch.Size([1, 8, 1560, 12, 128])
            salient_v_blocks = torch.gather(all_visual_v_blocks, 1, topk_block_indices_expanded)
        # else:
        #     salient_k_blocks = torch.empty(B, 0, tokens_per_block, H, D, device=hist_k.device, dtype=hist_k.dtype)
        #     salient_v_blocks = torch.empty(B, 0, tokens_per_block, H, D, device=hist_v.device, dtype=hist_v.dtype)

        # --- Step 3: Compress New BLOCKS via Prototype Merging ---
        
        if num_prototypes_in_blocks > 0 and num_new_blocks > 0:
            # Separate scores for new blocks
            new_block_scores = importance_scores_per_block[:, num_hist_blocks:]
            
            # Normalize scores to get weights for each block
            # torch.Size([1, 3])
            normalized_block_weights = torch.nn.functional.softmax(new_block_scores, dim=1)
            # torch.Size([1, 3])
            # Reshape for weighted average over blocks
            # Weights: [B, Num_New_Blocks, 1, 1, 1]
            # New K/V blocks: [B, Num_New_Blocks, Tokens_per_Block, H, D]
            weights_reshaped = normalized_block_weights.view(B, num_new_blocks, 1, 1, 1)

            # Compute weighted average of blocks to get the prototype block
            prototype_k_block = (new_k_blocks * weights_reshaped).sum(dim=1, keepdim=True)
            # torch.Size([1, 1, 1560, 12, 128])
            prototype_v_block = (new_v_blocks * weights_reshaped).sum(dim=1, keepdim=True)
            # Shape of prototype_k_block is [B, 1, Tokens_per_Block, H, D]
        # else:
        #     prototype_k_block = torch.empty(B, 0, tokens_per_block, H, D, device=new_k.device, dtype=new_k.dtype)
        #     prototype_v_block = torch.empty(B, 0, tokens_per_block, H, D, device=new_v.device, dtype=new_v.dtype)
            
        # --- Step 4: Construct the Final Compressed Cache in Block and Token Views ---

        # Concatenate salient historical blocks and the new prototype block(s)
        compressed_k_blocks = torch.cat([salient_k_blocks, prototype_k_block], dim=1)
        # torch.Size([1, 9, 1560, 12, 128])
        compressed_v_blocks = torch.cat([salient_v_blocks, prototype_v_block], dim=1)
        
        # Reshape back to the flat token view for the next model input
        final_num_tokens = memory_budget_in_blocks * tokens_per_block
        compressed_k = compressed_k_blocks.reshape(B, final_num_tokens, H, D)
        compressed_v = compressed_v_blocks.reshape(B, final_num_tokens, H, D)
        
        # Create the new cache dictionary for this layer
        new_layer_cache = {
            "k": compressed_k,
            "v": compressed_v,
            "local_end_index": torch.tensor([final_num_tokens], device=kv_cache['k'].device, dtype=torch.long)
        }

        return new_layer_cache
    ...
    def _forward_inference(
        ...
        if kv_cache is not None and cache_update_infos and update_cache:
            self._apply_cache_updates(kv_cache, cache_update_infos, kv_bank, crossattn_cache, update_bank=update_bank)
