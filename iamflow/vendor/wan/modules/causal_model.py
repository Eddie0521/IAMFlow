...
class CausalWanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self._temp_k: Optional[torch.Tensor] = None
        self._temp_v: Optional[torch.Tensor] = None
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            use_bank = iam_bank_length > 0
            # Dummy values for cache_update_info (bank is managed externally by IAM)
            local_start_index_bank = 0
            local_end_index_bank = 0
            current_end_bank = 0
            ...
            if self.local_attn_size != -1 and (current_end > cache_global_end) and (
                ...
                cache_k = kv_cache["k"]
                if self._temp_k is None or self._temp_k.shape != cache_k.shape:
                    self._temp_k = cache_k.clone()
                    self._temp_v = kv_cache["v"].clone()
                else:
                    self._temp_k.copy_(cache_k)
                    self._temp_v.copy_(kv_cache["v"])
                temp_k = self._temp_k
                temp_v = self._temp_v
