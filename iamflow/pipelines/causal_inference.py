...
from iamflow.utils.memory import gpu, get_cuda_free_memory_gb, DynamicSwapInstaller, move_model_to_device_with_memory_preservation, log_gpu_memory
...
class CausalInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        self.record_interval = getattr(args, "model_kwargs", {}).get("record_interval", 3)
        self.args = args
    ...
    def _initialize_kv_bank(self, batch_size, dtype, device, kv_bank1_size):
        ...
        kv_bank1 = []
        if DEBUG:
            print(f"rank {dist.get_rank()} initialize kv bank with batch_size: {batch_size}, kv_bank1_size: {kv_bank1_size}")
        for _ in range(self.num_transformer_blocks):
            kv_bank1.append({
                "k": torch.zeros([batch_size, kv_bank1_size, 12, 128], dtype=dtype, device=device),
                # "k_": torch.zeros([batch_size, kv_bank1_size, 12, 128], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, kv_bank1_size, 12, 128], dtype=dtype, device=device),
                "k_new": torch.zeros([batch_size, self.frame_seq_length, 12, 128], dtype=dtype, device=device),
                # "k_new_": torch.zeros([batch_size, self.frame_seq_length, 12, 128], dtype=dtype, device=device),
        ...
        self.kv_bank1 = kv_bank1  # always store the clean cache
