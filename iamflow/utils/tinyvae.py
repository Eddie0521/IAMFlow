...
def apply_decoder_with_memblocks_streaming(model, x, state=None, show_progress_bar=False):
    """Apply the TinyVAE decoder sequentially while preserving cross-chunk state."""
    assert x.ndim == 5, f"TAEHV operates on NTCHW tensors, but got {x.ndim}-dim tensor"
    N, T, C, H, W = x.shape
    if state is None:
        state = {
    ...
    mem = state["mem"]
    if len(mem) != len(model):
        raise ValueError("Decoder stream state does not match model depth")
    ...
    while work_queue:
        ...
        if isinstance(b, MemBlock):
            ...
            if past is None:
                ...
                if past.device != xt.device or past.dtype != xt.dtype:
                    past = past.to(device=xt.device, dtype=xt.dtype)
                    mem[i] = past
                xt_new = b(xt, past)
                if past.shape == xt.shape:
                    past.copy_(xt)
                else:
                    mem[i] = xt.detach().clone()
            work_queue.insert(0, TWorkItem(xt_new, i + 1))
        elif isinstance(b, TPool):
            raise ValueError("Decoder streaming does not support TPool blocks")
        elif isinstance(b, TGrow):
            xt = b(xt)
            NT, C, H, W = xt.shape
            for xt_next in reversed(xt.view(N, b.stride * C, H, W).chunk(b.stride, 1)):
...
class TAEHV(nn.Module):
    ...
    def clear_decode_stream(self):
        self._decode_stream_state = None

    def decode_video_stream(self, x, reset=False, show_progress_bar=False):
        """Decode a latent chunk while reusing the decoder's causal state."""
        if reset or getattr(self, "_decode_stream_state", None) is None:
            self._decode_stream_state = {
                "mem": [None] * len(self.decoder),
                "produced_frames": 0,
            }
        raw, self._decode_stream_state = apply_decoder_with_memblocks_streaming(
            self.decoder,
            x,
            state=self._decode_stream_state,
            show_progress_bar=show_progress_bar,
        )
        skip = max(0, self.frames_to_trim - self._decode_stream_state["produced_frames"])
        self._decode_stream_state["produced_frames"] += raw.shape[1]
        return raw[:, skip:]
